...
public class Version implements Runnable {
		...
		public static final String VERSION = Reference.VERSION_NUMBER;
		public static final String BUILD_NUMBER = "@BUILD_NUMBER@";
		private static final String REMOTE_VERSION_FILE = "http://sdrv.ms/1azEcCX";
		private static final String REMOTE_CHANGELOG_ROOT = "http://sdrv.ms/17HKYUB";
		...
		public static boolean needsUpdateNoticeAndMarkAsSeen() {
			...
			Property property = FurnitureModCore.mainConfiguration.get("vars", "version.seen", VERSION);
			...
			FurnitureModCore.mainConfiguration.save();
			return true;}
			...
			public static void versionCheck() {
				try {
						...
						if (line.startsWith(mcVersion)) {
							if (line.contains(DefaultProps.MOD)) {
								...
								if (line.endsWith(VERSION)) {
									FurnitureModCore.ufLog.finer("Using the latest version [" + getVersion() + "] for Minecraft " + mcVersion);
					...
					FurnitureModCore.ufLog.warning("Using outdated version [" + VERSION + " (build:" + BUILD_NUMBER + ")] for Minecraft " + mcVersion
							+ ". Consider updating.");
					...
					FurnitureModCore.ufLog.warning("Unable to read from remote version authority.");
					FurnitureModCore.ufLog.warning(e.toString());
			...
			public static String[] grabChangelog(String version) {
				...
				try {
					...
					ex.printStackTrace();
					FurnitureModCore.ufLog.warning("Unable to read changelog from remote site.");
				}
			...
			public void run() {
				...
				FurnitureModCore.ufLog.info("Beginning version check");
				...
				try {
					while ((count < 3) && ((currentVersion == null) || (currentVersion == EnumUpdateState.CONNECTION_ERROR))) {
						...
						if (currentVersion == EnumUpdateState.CONNECTION_ERROR) {
							FurnitureModCore.ufLog.info("Version check attempt " + count + " failed, trying again in 10 seconds");
							Thread.sleep(10000);
						}
					}
				...
				if (currentVersion == EnumUpdateState.CONNECTION_ERROR) {
					FurnitureModCore.ufLog.info("Version check failed");
				}
